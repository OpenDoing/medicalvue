...
function singleVali(obj, mode){ 
	...
	if(type == "truename" && isError){
		isError = !new RegExp('^[\u30A0-\u30FF\u3040-\u309F\u3400-\u4DB5\u2E80-\u9FBF]{1,8}[\.\u2027]{0,1}[\u30A0-\u30FF\u3040-\u309F\u3400-\u4DB5\u2E80-\u9FBF]{1,8}B{0,2}$').test(value);
	}
    function stopSubmit(){
    	if(mode){
    		acTips(tips.msg);
...
function userSingleVali(obj){ 
	var ele = $(obj),
	    ...
	    pattern = {
			...
			'num': '^[1-9][0-9]*$',
			'attack': '(\<script\>)|(\<SCRIPT\>)',
			// 'phone': '^0?(13[0-9]|14[5|7|9]|15[0-9]|16[1|2|4|5|6|7]|17[0-8]|18[0-9]|19[0-9])[0-9]{8}$',
			'phone': '^0?(^1[3|5|8|9][0-9]|14[5|6|7|8|9]|16[1|2|4|5|6|7]|17[0-8])[0-9]{8}$',
			'password': "^(?![0-9]+$)(?![a-zA-Z]+$)[0-9a-zA-Z]+$", // 包含且只能有数字和字母
			//'password': "^(?=.{6,20}$)(?![0-9]+$)(?![a-zA-Z]+$)[0-9a-zA-Z!\"#$%&'()*+,-./:;<=>?@\\[\\]^_`{|}~]+$", // 密码不能单独为数字、字母、特殊字符
			'idcard': '[0-9]{17}[0-9xX]{1}',
			'truename': '^[\u30A0-\u30FF\u3040-\u309F\u3400-\u4DB5\u2E80-\u9FBF]{2,7}B{0,2}$',
			'email': '[A-Z0-9a-z._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,4}',
			'money': '^[0-9]+(.[0-9]{1,2})?$',
			'contactway': '^(1[3|4|5|7|8][0-9]{9})$|^([A-Z0-9a-z._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,4})$'
	    },
...
(function(){
	if($(".editaccount").length > 0){
		...
		$("#card").on("change", function(){
			var type = $("#card option:selected").text();
            $("#cardnum").attr('placeholder', "请填写您本人的证件号码");
			if(type == "身份证"){				
				$(".js-birth,.js-sex").hide().find('.js-input').attr('data-required','');
				$("#sex").addClass("hidden");
                ...
                $(".js-birth,.js-sex").show().find('.js-input').attr('data-required','true');
                if($(".birth").length > 0){
                ...
                $("#cardnum").attr('placeholder', "例如：军1234567");
            }else if(type == '台胞证'){
                $(".js-birth,.js-sex").show().find('.js-input').attr('data-required','true');
                if($(".birth").length > 0){
                    $("#birthday").val($(".birth:last").text());
